"""
LLM-based Inference for Smart Contract Security Auditor.

Uses OpenAI-compatible client with Hugging Face Inference API.
Environment variables: API_BASE_URL, MODEL_NAME, HF_TOKEN
Structured stdout logs follow the [START], [STEP], [END] format.
"""
    ...
    for attempt in range(max_retries):
        try:
            ...
            if attempt < max_retries - 1:
                ...
                print(f"  [LLM ERROR] Failed after {max_retries} attempts: {err_msg[:80]}", flush=True)
...
def main():
    ...
    print(f"\n{'='*60}", flush=True)
    print(f"Overall average score: {avg_score:.3f}", flush=True)
    print(f"{'='*60}", flush=True)
